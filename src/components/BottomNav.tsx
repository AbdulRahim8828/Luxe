...
const BottomNav = () => {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-50 border-t border-gray-200 safe-area-inset-bottom">
      <div className="flex justify-around items-center max-w-full mx-auto px-2 py-0.5">
        {navigation.map((item) => (
          <Link
            key={item.name}
            to={item.href}
          ...
          >
            <item.icon className={`${isActive(item.href) ? 'w-5 h-5' : 'w-4 h-4'} transition-all duration-200`} />
            <span className={`text-[10px] mt-0.5 font-medium ${isActive(item.href) ? 'font-semibold' : ''}`}>{item.name}</span>
          </Link>
        ))}
      </div>
    </nav>
